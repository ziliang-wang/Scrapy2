...
class GuaziProjectSpiderMiddleware(object):
...
class GuaziProjectDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class GuaziDownloaderMiddleware:
    ...
    def __init__(self):
        with open('/scrapy/guazi_project/guazi_project/cc3.js', 'r', encoding='utf-8') as f:
            self.f_read = f.read()

    def process_response(self, request, response, spider):
        if '正在打开中,请稍后...' in response.text:
            re_res = re.search(r'anti\(\'(.*?)\'\,\'(.*?)\'\)', response.text, re.S).groups()
